	...
	create() {
		...
		const GetCatalogItemsCallback = (error, result) => {
			...
			store.items.forEach((item, i) => {
				...
				const priceText = store.add.text(16, 200 + i * 100, `${item.VirtualCurrencyPrices.CL} Clicks`, {
				...
				nameText.setInteractive({ useHandCursor: true }).on("pointerdown", () => {
					PlayFabClient.PurchaseItem(
						{ ItemId: item.ItemId, Price: item.VirtualCurrencyPrices.CL, VirtualCurrency: "CL" },
						(error, result) => {
							console.log(result);
